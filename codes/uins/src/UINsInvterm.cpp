...
void UINsInvterm::UpdateINsRes()
{
	...
	for (int cId = 0; cId < ug.nCell; ++cId)
	{
		...
		for (int iFace = 0; iFace < fn; ++iFace)
		{
			...
			if (ug.cId == ug.lc)
			{
				iinv.mu[ug.cId] += -iinv.ai[0][ug.fId] * (iinv.up[ug.rc] - iinv.uc[ug.rc]);  //矩阵非零系数，动量方程中与主单元相邻的单元面通量
			...
			{
				iinv.mu[ug.cId] += -iinv.ai[1][ug.fId] * (iinv.up[ug.lc] - iinv.uc[ug.lc]);  //矩阵非零系数，动量方程中与主单元相邻的单元面通量
